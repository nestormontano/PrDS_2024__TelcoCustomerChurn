...
data %>% count(Churn, StreamingMovies) %>% mutate(porc = n / sum(n)) %>%
  ggplot(aes(StreamingMovies, n, fill = Churn)) + geom_bar(stat =
'identity') + geom_text(aes(label = scales::percent(porc), y = n, vjust
= 1)) + labs(title = "Churn vs StreamingMovies", x = "StreamingMovies", y
= "Count")
